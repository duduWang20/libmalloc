 ...
 *
 * 1) Old malloc (before the scalable malloc implementation in this file existed).
...
//Emacs（/ˈiːmæks/，源自Editor MACroS，宏编辑器），是一个文本编辑器家族，
//具有强大的可扩展性，在程序员和其他以技术工作为主的计算机用户中广受欢迎。
/*
 * malloc_freezedry() records all of the szones in use, so that they can be
 * partially reconstituted 重组 by malloc_jumpstart().  Due to the differences
 ...
 *
 * Due to the flexibility provided by the zone registration mechanism,
 * it is impossible to implement generic freezedrying for any zone type.
 * This code only handles applications that use the szone allocator,
 * so malloc_freezedry() returns 0 (error) if any non-szone zones are encountered.
 */
